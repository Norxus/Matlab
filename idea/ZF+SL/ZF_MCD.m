...
for jj=1:IterationTimes
    ...
    for ii = 1:length(Eb) %计算多个信噪比情况下的输出Y 
       ...
       %尝试加入ZF检测器
            Y_ZF = (1/NT)*((pinv(H)*Y)/norm((pinv(H)*Y)));
            S_ZF_1_temp = zeros(1,(M)^NT);
            for i = 1 : (M)^NT
                S_ZF_1_temp(i) = norm(x_training(:,i)-Y_ZF);
            end
             S_ZF_1_Index = find(S_ZF_1_temp==min(S_ZF_1_temp));
             S_ZF_1_temp(S_ZF_1_Index) = 100;
             S_ZF_2_Index = find(S_ZF_1_temp==min(S_ZF_1_temp)); 
             
             %生成所有的可能的索引
             x_nearest = x_training(:,S_ZF_1_Index);
             x_secondNearst = x_training(:,S_ZF_2_Index);
             x_temp = zeros(2,NT);
             for i = 1 : NT
                 x_temp(:,i) = [x_nearest(i),x_secondNearst(i)].';
             end
             [x_1,x_2] = ndgrid(x_temp(:,1),x_temp(:,2));
             NT_Factor = 1 : NT;
             x_candidate = ([x_1(:),x_2(:)].*NT_Factor).';
             [x_candidate_isnot,x_candidate_id] = ismember(unique(sum(x_candidate)),sum(x_candidate));
             %找出不同的列
             x_candidate_final = x_candidate(:,x_candidate_id);
             [x_candidate_final_Index_isnot,x_candidate_final_Index]=ismember(sum(x_candidate_final),sum(x_training.*NT_Factor'));
             
             
            YR = [sign(real(Y)); sign(imag(Y))]; 
            vectorDetection = zeros(1,length(x_candidate_final_Index));
            for i = 1 : length(x_candidate_final_Index)
                detectionData = norm(YR - E_y(:,x_candidate_final_Index(i)));
                vectorDetection(i) = detectionData;
            end
            [minValue,minIndex] = min(vectorDetection);
            X_finalDetection = x_training(:,x_candidate_final_Index(minIndex));
